...
public class Main extends Application
{
   ...
   Text money=new Text("Current Funds: " + Player.getInstance().getMoney());
      ...
      public void handle(ActionEvent e)      
      {
         if(e.getSource() == start)
         {
            ...
            gameCanvas.requestFocus();
             //starts game with selected weapon
             if(Player.getInstance().hasSpeckWeapon)
             {
                 Player.getInstance().currentWeapon = Player.WeaponType.SPECK;
             } else if (Player.getInstance().hasSideWeapon)
             {
                 Player.getInstance().currentWeapon = Player.WeaponType.SIDES;
             }
            current = new Level();
            current.Start();
         }
   ...
   public class AnimationHandler extends AnimationTimer
   {
      ...
      {
          //updates the current funds field when in the shop
         money.setText("Current Funds: " + Player.getInstance().getMoney());
         if(lastTime != -1)
         {
